...
class mparametro{
	...
	function selpar1($codpar){
		...
		function selval1(){
		$sql = "SELECT parametro.nompar, valor.nomval, valor.codval, valor.valor_fijo FROM parametro RIGHT JOIN valor 
		ON parametro.codpar=valor.codpar ORDER BY parametro.codpar";
		$conexionBD = new conexion();
		$conexionBD->conectarBD();
		$data = $conexionBD->ejeCon($sql,0);
		return $data;
		...
		$sql = "SELECT parametro.nompar ,valor.codval, valor.nomval, valor.valor_fijo  FROM parametro RIGHT JOIN valor ON parametro.codpar=valor.codpar";
		if($filtro)
